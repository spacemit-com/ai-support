...
// 检测线程
void Detection(SharedDataLoader& dataloader, Detector& detector) {
  cv::Mat frame; 
...
// 预览线程
void Preview(SharedDataLoader& dataloader, Detector& detector) {
  cv::Mat frame;
  while (dataloader.ifenable())
  {
...
int main(int argc, char* argv[])
{
  ...
  {
    std::cout<<"run with "<<argv[0]<<" <modelFilepath> <labelFilepath> <input> <inputType> (video or cameraId)" <<std::endl;
    return 0;
  }
