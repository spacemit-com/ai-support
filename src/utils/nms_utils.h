...
static void hard_nms(std::vector<Boxf> &input, std::vector<Boxf> &output,
...
static void blending_nms(std::vector<Boxf> &input, std::vector<Boxf> &output,
                         float iou_threshold, unsigned int topk) {
  if (input.empty()) return;
  std::sort(input.begin(), input.end(),
            [](const Boxf &a, const Boxf &b) { return a.score > b.score; });
...
static void offset_nms(std::vector<Boxf> &input, std::vector<Boxf> &output,
                       float iou_threshold, unsigned int topk) {
  if (input.empty()) return;
  std::sort(input.begin(), input.end(),
            [](const Boxf &a, const Boxf &b) { return a.score > b.score; });
